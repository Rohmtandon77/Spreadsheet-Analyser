...
def eval_by_type(categoried_llm_inference_results, metric_eval_engine):
    ...
    for llm_reasoning_method, result in categoried_llm_inference_results.items():
        ...
        # calculate the metric for each qtype (skip if no samples)
        da_em_part_results = da_em_with_error_10_part_results = da_rouge_part_results = []
        # == FactChecking ==
        fc_results = qtype_dict.get('FactChecking', [])
        if fc_results:
        ...
        # == NumericalReasoning ==
        nr_results = qtype_dict.get('NumericalReasoning', [])
        if nr_results:
        ...
        # == DataAnalysis ==
        da_results = qtype_dict.get('DataAnalysis', [])
        if da_results:
            ...
            da_em = metric_eval_engine.compute(
                references=da_em_references, predictions=da_em_predictions)['EM'] if da_em_total else 0.0
            ...
            da_em_with_error_10 = (metric_eval_engine.compute(
                ...
                if da_em_with_error_10_total else 0.0)
            ...
            da_rouge = (metric_eval_engine.compute(
                ...
                if da_rouge_total else 0.0)

            denom = da_em_total + da_em_with_error_10_total + da_rouge_total
            da_mix_metric = (da_em * da_em_total + da_em_with_error_10 * da_em_with_error_10_total + da_rouge * da_rouge_total) / denom if denom else 0.0
        ...
        # == Visualization ==
        vis_results = qtype_dict.get('Visualization', [])
        if vis_results:
            vis_metric_scores = {}
        ...
        if overall_em_with_error_10_total:
            ...
            overall_em_with_error_10_score = 0.0

        overall_rouge_total = len(overall_rouge_results)
        if overall_rouge_total:
            ...
            overall_rouge_score = 0.0

        overall_pass_1_total = len(overall_pass_1_results)
        if overall_pass_1_total:
            overall_pass_1_predictions = [result["parsed_result"]
                                          ['parsed_prediction'] for result in overall_pass_1_results]
            overall_pass_1_score = overall_pass_1_predictions.count(True) / overall_pass_1_total
        else:
            overall_pass_1_score = 0.0

        total_n = overall_em_total + overall_em_with_error_10_total + overall_rouge_total + overall_pass_1_total
        overall_score = (overall_em_score * overall_em_total + overall_em_with_error_10_score * overall_em_with_error_10_total + overall_rouge_score * overall_rouge_total + overall_pass_1_score * overall_pass_1_total) / total_n if total_n else 0.0
